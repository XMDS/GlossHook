#ifndef ANDROID_GLOSSHOOK_H
#define ANDROID_GLOSSHOOK_H
...
#define GET_INST_SET(addr) (addr & 1 ? i_set::$THUMB : i_set::$ARM)
	...
	* is_dlclose: Do you want to use the "dlclose" function ?
	* return: 0 succeeded, not 0 failed (Return dlclose).
	*/
...
#define RE_ORIG_ADDR(original) decltype(original)(GlossGetResultAddr((void*)original))
	...
	*/
	...
	{
		return GotHook(reinterpret_cast<void*>(addr), reinterpret_cast<void*>(func), reinterpret_cast<void**>(original));
	}
	template <class A, class B>
	inline static void GOTHook(A addr, B func)
	{
		return GotHook(reinterpret_cast<void*>(addr), reinterpret_cast<void*>(func), NULL);
	}
